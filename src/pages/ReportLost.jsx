...
import { MapPin, Camera, Smartphone, Tag, ArrowRight, Check } from 'lucide-react';

const steps = [
    { id: 1, title: 'Item Details', icon: Tag },
    { id: 2, title: 'Location & Time', icon: MapPin },
    { id: 3, title: 'Contact & Photo', icon: Camera },
];
...
const ReportLost = () => {
            ...
            case 1:
                return (
                    <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} className="space-y-5">
                        ...
                        <div>
                            <label className="block text-xs font-semibold text-text-muted uppercase tracking-wider mb-2">Item Name</label>
                            <input type="text" className={inputClass} placeholder="e.g. iPhone 14 Pro" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} />
                        </div>
                        <div>
                            <label className="block text-xs font-semibold text-text-muted uppercase tracking-wider mb-2">Description</label>
                            <textarea className={`${inputClass} h-28 resize-none`} placeholder="Describe scratches, stickers, markings..." value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })} />
                        </div>
                    </motion.div>
                );
            case 2:
                return (
                    <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} className="space-y-5">
                        <div className="h-44 bg-background rounded-lg border border-divider flex items-center justify-center">
                            <div className="flex items-center gap-2 text-text-muted text-sm">
                                <MapPin size={18} className="text-primary" />
                                <span>Pin last known location</span>
                            </div>
                        </div>
                        <div>
                            <label className="block text-xs font-semibold text-text-muted uppercase tracking-wider mb-2">Last Seen Location</label>
                            <input type="text" className={inputClass} placeholder="e.g. Central Park, near the fountain" value={formData.location} onChange={e => setFormData({ ...formData, location: e.target.value })} />
                        </div>
                        <div>
                            <label className="block text-xs font-semibold text-text-muted uppercase tracking-wider mb-2">Date & Time Lost</label>
                            <input type="datetime-local" className={inputClass} value={formData.date} onChange={e => setFormData({ ...formData, date: e.target.value })} />
                        </div>
                    </motion.div>
                );
            case 3:
                    ...
                    <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} className="space-y-5">
                        ...
                        <div className="bg-primary/5 border border-primary/20 rounded-lg p-5">
                            <div className="flex items-center gap-3 mb-3">
                                ...
                                <div>
                                    <h4 className="font-bold text-sm text-text-primary">AI Scan Running</h4>
                                    <p className="text-xs text-primary">2 potential matches found</p>
                                </div>
                            </div>
                            <div className="flex gap-3">
                                {[1, 2].map(i => (
                                    <div key={i} className="flex-1 h-16 bg-background rounded-lg border border-divider flex items-center justify-center">
                                        <span className="text-xs text-text-muted">Locked — submit to view</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div>
                            <label className="block text-xs font-semibold text-text-muted uppercase tracking-wider mb-2">Contact Email / Phone</label>
                            <input type="text" className={inputClass} placeholder="How can finders reach you?" value={formData.contact} onChange={e => setFormData({ ...formData, contact: e.target.value })} />
                        </div>

                        <label className="flex items-center gap-3 cursor-pointer">
                            <div
                                className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-colors ${formData.reward ? 'bg-primary border-primary' : 'border-divider'}`}
                                onClick={() => setFormData({ ...formData, reward: !formData.reward })}
                            >
                                {formData.reward && <Check size={12} className="text-white" />}
                            </div>
                            <span className="text-sm text-text-muted">I'm offering a reward for this item</span>
                        </label>
                    </motion.div>
                );
            default: return null;
    ...
    return (
        <div className="min-h-screen pt-32 pb-20 px-6 max-w-xl mx-auto">
            <div className="mb-10">
                <h1 className="text-2xl font-bold text-text-primary mb-1">Report a Lost Item</h1>
                <p className="text-text-muted text-sm">Help us reunite you with your belongings.</p>
            </div>
